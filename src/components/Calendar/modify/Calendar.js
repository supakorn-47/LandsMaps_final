  ...
  _createClass(Calendar, [{
    ...
    value: function componentDidUpdate(prevProps) {
      ...
      if (prevProps.value !== this.props.value && (!this.viewStateChanged || this.state.overlayVisible)) {
        ...
        // if (this.isKeydown !== undefined) {
        //   console.log('value -->', value);
        //   if (value !== null){
        //     _this2.props.value = new Date(parseInt(value.getFullYear()) - 543, value.getMonth(), value.getDate());
        //   }
        // }
        this.updateInputfield(this.props.value);
      }
    ...
    value: function onInputBlur(event) {
      ...
      this.setState({
        ...
        if (!_this4.props.keepInvalid) {
          // if (_this4.props.value !== null && _this4.props.value !== '') {
    ...
    value: function onInputKeyDown(event) {
      this.isKeydown = true;
      // console.log('onInputKeyDown -->', event.which);
      ...
      try {
        ...
        if (this.isValidSelection(value)) {
          if (this.props.showTime === true) {
            value = new Date(parseInt(value.getFullYear()) - 543, value.getMonth(), value.getDate(), value.getHours(), value.getMinutes(), value.getSeconds(), value.getMilliseconds());
          } else {
            value = new Date(parseInt(value.getFullYear()) - 543, value.getMonth(), value.getDate());
          }
    ...
    value: function onClearButtonClick(event) {
      ...
      //เป็นปุ่มล้างเหมือนเดิม 24/05/2564(jane)
      this.updateModel(event, null);
      this.updateInputfield(null);
      this.hideOverlay(this.reFocusInputField);

      // ปุ่มล้าง เปลี่ยนเป็น ตกลง
    ...
    value: function updateInputfield(value) {
      ...
      try {
        if (value !== null && value !== '') {
          if (this.props.showTime === true) {
            // console.log('data -->', value.getFullYear(), '||', value.getMonth(), '||', value.getDate());
            // console.log('time -->', value.getHours(), '||', value.getMinutes(), '||', value.getSeconds(), '||', value.getMilliseconds());
            value = new Date(parseInt(value.getFullYear()) + 543, value.getMonth(), value.getDate(), value.getHours(), value.getMinutes(), value.getSeconds(), value.getMilliseconds());
            // console.log('value -->', value);
          } else {
            value = new Date(parseInt(value.getFullYear()) + 543, value.getMonth(), value.getDate());
            ...
            //   return this.updateInputfield(value);
      ...
      if (value) {
        ...
        //! TOM11 ปีอธิกสุรทิน - Jane
        let _returnValue = formattedValue;
        let arr = this.formatDateTime(_default).split("/");
        // To พศ
        arr[2] = parseInt(arr[2]) + 543;
        if (this.props.showTime === true) {
          _returnValue = `${String(parseInt(arr[0])).padStart(2, '0')}/${arr[1]}/${parseInt(arr[2])} ${value.getHours()}:${value.getMinutes()}:${value.getSeconds()}`
        } else {
          _returnValue = `${String(parseInt(arr[0])).padStart(2, '0')}/${arr[1]}/${parseInt(arr[2])}`
        }
        this.inputElement.value = _returnValue;
        //!END
      } else {
    ...
    value: function renderButtonBar() {
      ...
      if (this.props.showButtonBar) {
        ...
        var todayClassName = (0, _classnames.default)('p-button-rounded', this.props.todayButtonClassName);
        var clearClassName = (0, _classnames.default)('p-button-rounded', this.props.clearButtonClassName);
        return /*#__PURE__*/_react.default.createElement("div", {
...
_defineProperty(Calendar, "defaultProps", {
  ...
  showButtonBar: false,
  todayButtonClassName: 'p-button-info',
  clearButtonClassName: 'p-button-primary',
  autoZIndex: true,
  baseZIndex: 0,
  appendTo: null,
  tooltip: null,
