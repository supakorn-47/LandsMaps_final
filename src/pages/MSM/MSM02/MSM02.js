...
import { formatDateTH2, formatDateTH_full2, formatDateAPI } from '../../../utils/DateUtil';
import { getTextMenu } from '../../../utils/MenuUtil';

//PAGE
import MSM02Search from './MSM02Search';
import MSM02List from './MSM02List';
import MSM02Dialog from './MSM02Dialog';
...
//PDF
import { generateTableMSM2, generatePdfOpenNewTab } from '../../../utils/PDFMakeUtil';
...
export default function MSM02() {
    ...
    useEffect(() => {
        ...
        //แหล่งข้อมูล
        masterService(`GetDataSource?mode=${0}&source_process=1`, {}, "GET").then(res => {
        ...
        //กลุ่มตาราง
        masterService(`GetTransferDataGroup?mode=${0}`, {}, "GET").then(res => {
    ...
    const onCreatePDFClick = async () => {
        ...
        if (data.length > 0) {
            ...
            var content = {
                ...
                content: [
                    generateTableMSM2(searchData, _arr),
                ],
    ...
    const onCreateExcelClick = () => {
        ...
        if (dataTable.length > 0) {
            ...
            let headerText = [{
                "text": "รายงานข้อมูล Log ถ่ายโอนข้อมูล" + "\n" + formatDateTH_full2(searchData.start_date, true) + " ถึง " + formatDateTH_full2(searchData.end_date, true),
            ...
            for (let i = 0; i < dataRow.length; i++) {
                ...
                dataExcel.push([
                    ...
                    { value: dataRow[i].log_start_dtm ? formatDateTH2(dataRow[i].log_start_dtm, true) : '', style: text_Center },
                    { value: dataRow[i].log_end_dtm ? formatDateTH2(dataRow[i].log_end_dtm, true) : '', style: text_Center },
                    { value: dataRow[i].source_name ? dataRow[i].source_name : '', style: text_Center },
                    { value: dataRow[i].transfer_source_table ? dataRow[i].transfer_source_table : '', style: text_Center },
                    { value: dataRow[i].target_name ? dataRow[i].target_name : '', style: text_Default },
                    { value: dataRow[i].source_record_num ? dataRow[i].source_record_num : '', style: text_Default },
                    { value: dataRow[i].target_record_num ? dataRow[i].target_record_num : '', style: text_Default },
                    { value: dataRow[i].schedule_mode ? dataRow[i].schedule_mode : '', style: text_Default },
                    { value: dataRow[i].transfer_status ? dataRow[i].transfer_status : '', style: text_Default },
                ])

            }
            exportAsExcel(dataExcel, "MSM02", [{ s: { r: 3, c: 0 }, e: { r: 0, c: 9 } }]);
        }
        else {
            showMessages('warn', `เกิดข้อผิดพลาด`, 'ไม่พบข้อมูลส่งออก');
            setLoading(false)
    ...
    const exportFile = (dataExcel) => {
        ...
        let filename = "MSM02_" + dateFormat(new Date().setFullYear(2564), 'yyyymmdd')
        ...
        <>
            ...
            <div className="search-container">
                ...
                <div className="">
                    ...
                    {dialog.dialog && (
                        <MSM02Dialog dialog={dialog} setDialog={setDialog} />
                    )}
            ...
            <div className="search-container">
                <MSM02List dataTable={dataTable} setDialog={setDialog} />
            </div>
