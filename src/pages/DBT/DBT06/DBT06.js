...
//PAGE
import DBT06Search from "./DBT06Search";
import DBT06List from "./DBT06List";

//SERVICE
import { DBT06GetDataList } from "../../../service/ServiceDBT/ServiceDBT06";
import { masterService } from "../../../service/ServiceMaster/MasterService";

//PDF
import { generatePdf, generateHead_DBT06 } from "../../../utils/PDFMakeUtil";
...
export default function DBT06() {
  ...
  const onDBT06GetDataList = () => {
    ...
    let data = {
      ...
      transfer_data_group_seq:
        searchData.transfer_data_group_seq === "0"
          ? 0
          : parseInt(searchData.transfer_data_group_seq),
    };
    DBT06GetDataList(data).then(
      (res) => {
        setLoading(false);
        if (res.status === 200) {
          let temp = [];
  ...
  const onCreatePDFClick = async () => {
    ...
    if (data.length > 0) {
      ...
      var content = {
        ...
        pageSize: "A4",
        content: [generateHead_DBT06(searchData), data2],
  ...
  const onCreateExcelClick = () => {
    ...
    if (dataTable.length > 0) {
      ...
      for (let i = 0; i < dataRow.length; i++) {
        ...
        dataExcel.push([
          {
            value: dataRow[i].order_no ? dataRow[i].order_no : "",
          ...
          {
            value: dataRow[i].transfer_data_group_name
              ? dataRow[i].transfer_data_group_name
              : "",
            style: styleBorderB,
          },
          {
            value: dataRow[i].transfer_data_group_process_seq
              ? dataRow[i].transfer_data_group_process_seq
              : "",
            style: styleBorderB_Center,
          },
          {
            value: dataRow[i].source_process ? dataRow[i].source_process : "",
            style: styleBorderB,
          },
          {
            value: dataRow[i].source_schema ? dataRow[i].source_schema : "",
            style: styleBorderB,
          },
          {
            value: dataRow[i].source_table ? dataRow[i].source_table : "",
            style: styleBorderB,
          },
          {
            value: dataRow[i].target_process ? dataRow[i].target_process : "",
            style: styleBorderB,
          },
          {
            value: dataRow[i].target_schema ? dataRow[i].target_schema : "",
            style: styleBorderB,
          },
          {
            value: dataRow[i].target_table ? dataRow[i].target_table : "",
            style: styleBorderB,
          },
          {
          ...
          {
            value: numberWithCommas(dataRow[i].total_record),
            style: styleBorderB_Right,
          },
          {
            value: dataRow[i].transfer_process_status
              ? dataRow[i].transfer_process_status
              : "",
            style: styleBorderB_Center,
          },
          {
            value: dataRow[i].log_desc ? dataRow[i].log_desc : "",
            style: styleBorderB_Center,
          },
          {
            value: dataRow[i].log_path ? dataRow[i].log_path : "",
            style: styleBorderB,
          },
        ]);
      }
  ...
  const exportFile = (dataExcel) => {
    ...
    let filename =
      "DBT06_" + dateFormat(new Date().setFullYear(2564), "yyyymmdd");
    ...
    <div className="page-wrapper">
      ...
      <CustomCard
        title={
          <PageHeader
            config={{
              title: "รายงานสรุปการถ่ายโอนข้อมูล",
              actionButton: (
                <div>
                  <Button
        ...
        body={
          <DBT06Search
            ...
            tableList={tableList}
            onDBT06GetDataList={onDBT06GetDataList}
          />
        }
      />
      <CustomCard>
        <DBT06List dataTable={dataTable} />
      </CustomCard>
