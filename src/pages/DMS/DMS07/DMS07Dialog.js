...
export default function DMS07Dialog({ dialog, setDialog, submitForm, optionPattern, submitted, setSubmitted, onUpdateNextTime }) {

  const [formObject, setFormObject] = useState({ SCHEDULE_TYPE: 1, SOURCE: 0, start_dtm: new Date() });
  ...
  useEffect(() => {
    ...
    if (dialog.dialog === true && dialog.action === 'แก้ไข') {
      ...
      if (dialog.data.job_pattern != null) {
        ...
        for (var i = 0; i < jobpattern.length; i++) {
          let fitter_jobpattern = optionPattern.find(y => y.transfer_data_seq === parseInt(jobpattern[i]))
          // console.log('fitter_jobpattern', fitter_jobpattern)
          arr_Temp.push(fitter_jobpattern)
          setSelectedTable(arr_Temp)
          setSelectTotal(arr_Temp.length)
        }
      }
      ...
      setFormObject({
        ...
        "start_dtm": (!dialog.data.start_dtm ? "" : new Date(dialog.data.start_dtm)),
    ...
    if (dialog.dialogTime === true) {
      setFormObject({
        ...dialog.data,
        "next_run_time": dialog.data.next_run_time === null ? "" : new Date(dialog.data.next_run_time)
      })
    }
  }, [dialog])
          ...
          <div className="p-col-6">
            <div>
              ...
              <Calendars
                ...
                showTime
              />
              {submitted && (formObject.start_dtm === null || formObject.start_dtm === "") && validateInputText('start_dtm', 'Start Date')}
            </div>
          </div>
        );
      } else if (type == 'Daily') {
        return (
          <div className="p-col-6">
            ...
            <div style={{ marginTop: 12 }}>
              ...
              <InputNumber value={formObject.interval_minute}
                onValueChange={(e) => onInputChange(e, "interval_minute")}
                mode="decimal" min={1} max={60} showButtons
          ...
          <div className="p-col-6">
            ...
            <div style={{ display: 'flex' }}>
              <div style={{ marginTop: 12, width: '49%' }}>
                ...
                <InputNumber value={formObject.interval_day_of_month}
                  onValueChange={(e) => onInputChange(e, "interval_day_of_month")}
                  mode="decimal" min={1} max={31}
                  showButtons
                  inputStyle={{ textAlign: 'right' }} />
      ...
      >
        <div className="p-grid" >
          <div className="p-col-12">
            <label>Job Detail<span style={{ color: "red" }}>*</span></label>
            <InputText
              value={formObject.job_detail}
              onChange={(e) => onInputChange(e, "job_detail")}
            />
            {submitted && formObject.job_detail === "" && validateInputText('job_detail', 'Job Detail')}
          </div>

          <div className="p-col-6">
            <div>
              <label>Schedule Mode<span style={{ color: "red" }}>*</span></label>
              <Dropdown
                appendTo={document.body}
                ...
                placeholder=""
              />
              {submitted && formObject.schedule_mode === "0" && validateInputText('schedule_mode', 'Schedule Mode')}
            </div>
            <div style={{ marginTop: 12 }}>
              ...
              />
              {submitted && formObject.schedule_type === null && validateInputText('schedule_type', 'Schedule Type')}
            </div>
          </div>
        ...
        <div className="p-grid" >
          <div className="p-col-12" style={{ marginTop: 6 }}>
            ...
            <div className="table-header">
              ...
              <span className="p-input-icon-right" style={{ width: '7%' }}> {"เลือก" + " " + selectTotal + " " + "รายการ"}</span>
            </div>
            <DataTable
            ...
            >
              ...
              <Column field="index" header="ลำดับ" style={{ width: '4em', textAlign: 'center' }}></Column>
              <Column field="source_name" header="แหล่งข้อมูล" style={{ wordWrap: 'break-word' }}></Column>
              <Column field="source_process" header="แหล่งข้อมูลต้นทาง" style={{ wordWrap: 'break-word' }}></Column>
              <Column field="source_schema" header="Schema ต้นทาง" style={{ wordWrap: 'break-word' }}></Column>
              <Column field="source_table" header="ตารางต้นทาง" style={{ wordWrap: 'break-word' }}></Column>
              <Column field="target_process" header="แหล่งข้อมูลปลายทาง" style={{ wordWrap: 'break-word' }}></Column>
              <Column field="target_schema" header="Schema ปลายทาง" style={{ wordWrap: 'break-word' }}></Column>
              <Column field="target_table" header="ตารางปลายทาง" style={{ wordWrap: 'break-word' }}></Column>
            </DataTable>
          </div>
        </div>
      </Dialog>
    ...
    const footer = () => {
      return (
        <div style={{ textAlignLast: 'end' }}>
          ...
          <Button label={dialog.action} icon="pi pi-check" onClick={() => onUpdateNextTime(formObject)} autoFocus className="p-button-rounded" />
        </div>
      );
    }
    return (
      ...
      >
        <div className="p-grid" >
          <div className="p-col-12">
            <label>วันเวลา Next Run Time<span style={{ color: "red" }}>*</span></label>
            <Calendars
              value={formObject.next_run_time}
              dateFormat={"dd/mm/yy"}
              showTime
              minDate={new Date()}
              onChange={(e) => setFormObject({ ...formObject, next_run_time: e.target.value })}
            />
            {submitted && (formObject.next_run_time === null || formObject.next_run_time === "") && validateInputText('next_run_time', 'วันเวลา Next Run Time')}
