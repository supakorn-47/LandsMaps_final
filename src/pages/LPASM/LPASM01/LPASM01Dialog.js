...
export default function LPASM01Dialog({
  ...
  const handleSubmit = () => {
    if (
      !formObject.register_type_name ||
      formObject.register_type_name.trim() === ""
    ) {
    ...
    return (
      <div className="dialog-footer-action-right">
        <Button
          label="ยกเลิก"
          icon="pi pi-times"
          onClick={() => setDialog({ dialog: false, action: "" })}
  ...
  const renderDialog = () => {
    return (
      <Dialog
        ...
        style={{ width: "35vw" }}
        footer={renderFooter()}
        onHide={() => setDialog({ dialog: false, action: "" })}
        blockScroll={true}
